...
UI_Slider::UI_Slider(int x, int y, UI_Type type, SDL_Rect scrollbar, SDL_Rect button, float width, UI_element* parent, j1Module* Observer) : UI_element(x, y, type, parent, Observer)
{
	...
	if (t == Slider_music) {
		...
		initial_v = static_cast<float>(x);
		Scrollbar_volume = scrollbar;

		max_v = initial_v + (Scrollbar_volume.w + 34);

		actual_pos_v = static_cast<float>(x + 89);
	...
	if (t == Slider_fx) {
		...
		initial_f = static_cast<float>(x);
		Scrollbar_fx = scrollbar;

		max_f = initial_f + (Scrollbar_fx.w + 34);

		actual_pos_f = static_cast<float>(x + 89);
...
bool UI_Slider::Mouse_Is_Moving() {
	...
	if (t == Slider_music) {
		...
		actual_pos_v = static_cast<float>(new_mouse_pos_music.x);
	}

	if (t == Slider_fx) {
		App->input->GetMousePosition(new_mouse_pos_fx.x, new_mouse_pos_fx.y);
		actual_pos_f = static_cast<float>(new_mouse_pos_fx.x);
	}
...
bool UI_Slider::Draw() {
	...
	if (t == Slider_music) {
		...
		App->render->Blit_UI(texture, static_cast<int>(actual_pos_v), static_cast<int>(pos.y - 4), &Button_Scrollbar_volume, SDL_FLIP_NONE, 0.0f);
		Debug();
	}

	if (t == Slider_fx) {
		App->render->Blit_UI(texture, pos.x, pos.y, &Scrollbar_fx, SDL_FLIP_NONE, 0.0f);
		App->render->Blit_UI(texture, static_cast<int>(actual_pos_f), static_cast<int>(pos.y - 4), &Button_Scrollbar_fx, SDL_FLIP_NONE, 0.0f);
		Debug();
	}
...
float UI_Slider::get_valors()
{
	...
	if (t == Slider_music) {
		int max_valor = static_cast<int>(max_v - initial_v);
	...
	if (t == Slider_fx) {
		int max_valor = static_cast<int>(max_f - initial_f);
		float min_valor = max_valor / 100.0f;
		float current;
...
void UI_Slider::Debug() {
	...
	if (t == Slider_music) {
		...
		if (App->gui->debug_UI == true) {
			...
			SDL_Rect button_scrollbar{ static_cast<int>(actual_pos_v), static_cast<int>(pos.y - 4), Button_Scrollbar_volume.w, Button_Scrollbar_volume.h };
		...
		if (App->gui->debug_UI == true) {
			...
			SDL_Rect button_scrollbar2{ static_cast<int>(actual_pos_f), static_cast<int>(pos.y - 4), Button_Scrollbar_fx.w, Button_Scrollbar_fx.h };
