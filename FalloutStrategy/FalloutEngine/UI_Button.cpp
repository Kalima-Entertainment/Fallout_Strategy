...
bool UI_Button::Update(float dt)
{
	...
	if (IsIntersection() == true) {
		...
		if (App->input->GetMouseButtonDown(SDL_BUTTON_LEFT)==KEY_DOWN) {
			...
			{
			Mix_PauseMusic();
			App->audio->PlayMusic("Assets/audio/music/Ultraradio/cloudnone-from-here-monstercat-release.ogg", 0.0F);
			}
			...
			{
			Mix_PauseMusic();
			App->audio->PlayMusic("Assets/audio/music/AyuwokiFM/michael-jackson-billie-jean.ogg", 0.0F);
			}
